...
Bundled character and composition imagery is not source-brand evidence or UI-style-token evidence. Its rough black outlines, cut-paper or sticker treatment, blocky controls, grain, paper texture, palette, and offset shadows carry no interface authority. Do not infer or reinterpret any of them as Fantasy Mouse source-brand constraints. Source-brand evidence exists only when supplied by the target product or user. Photographic-face/drawn-body contrast applies only to the character asset, not to interface materials.
...
Compare the candidate only against an allowed unrelated baseline: a user-supplied comparison, a packaged comparison baseline in the active plugin bundle, or another unrelated output created in the current run. Do not search prior conversations, memories, sibling repositories, or similarly named products for a baseline.

If no allowed baseline exists, emit this exact result on its own line:

`anti-template-baseline-unavailable`

This does not block delivery. Report the cross-output subgate as unavailable, complete the product-evidence provenance check, and do not claim the unavailable subgate passed.

Mark `template-leakage` when three or more of these repeat without product-specific justification: skeleton, character placement/scale, palette, material/texture, border/radius/shadow system, title/label treatment, component composition, decorative motifs, interaction arrangement.
