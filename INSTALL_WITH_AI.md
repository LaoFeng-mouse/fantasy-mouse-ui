...
Until the GitHub v0.2.0 Release and its digest are independently verified, the trusted release-candidate route is a local source build. 在 GitHub v0.2.0 Release 及其摘要完成独立验证前，受信任的发布候选路径是本地源码构建。Run `pnpm plugin:package`, retain the resulting archive identity, and install the verified `dist/plugin/fantasy-mouse-ui.zip`.

A downloaded ZIP may be treated as the current package only when the exact 24-entry inventory and applicable validators pass and the complete ZIP matches a trusted release-published SHA-256 digest, or is byte-for-byte identical to a separately trusted locally built archive. 下载的 ZIP 只有在精确 24 项清单与适用验证器全部通过，并且完整 ZIP 匹配可信发布流程公布的 SHA-256 摘要，或与另一个单独受信任的本地构建压缩包逐字节相同时，才可视为当前包。No remote v0.2.0 digest is asserted before that verification.
...
在 GitHub v0.2.0 Release 及其摘要完成独立验证前，受信任的发布候选路径是本地源码构建：从当前源码运行 pnpm plugin:package。固定的 https://github.com/LaoFeng-mouse/fantasy-mouse-ui/releases/tag/v0.1.0 是历史 v0.1.0 发布记录；https://github.com/LaoFeng-mouse/fantasy-mouse-ui/releases/latest 和 https://github.com/LaoFeng-mouse/fantasy-mouse-ui/releases/latest/download/fantasy-mouse-ui.zip 是可变发现端点，不建立版本或信任。下载 ZIP 必须通过精确 24 项清单与适用验证器，并匹配可信发布流程公布的 SHA-256 摘要，或与另一个单独受信任的本地构建压缩包逐字节相同。最终目标必须不存在或为空；存在安装时停止并报告，更新走单独的备份替换或宿主重装流程。
...
Until the GitHub v0.2.0 Release and its digest are independently verified, the trusted release-candidate route is a local source build: run pnpm plugin:package from the current source. The fixed https://github.com/LaoFeng-mouse/fantasy-mouse-ui/releases/tag/v0.1.0 URL is the historical v0.1.0 release record; https://github.com/LaoFeng-mouse/fantasy-mouse-ui/releases/latest and https://github.com/LaoFeng-mouse/fantasy-mouse-ui/releases/latest/download/fantasy-mouse-ui.zip are mutable discovery endpoints and establish neither version nor trust. A downloaded ZIP must pass the exact 24-entry inventory and applicable validators and match a trusted release-published SHA-256 digest, or be byte-for-byte identical to a separately trusted locally built archive. Final must be absent or empty; stop and report an existing installation, and handle updates only through a separate backup/replacement or host reinstall flow.
